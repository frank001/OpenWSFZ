...
- [ ] 1.1 Add `ftx_extract_likelihood_at()` to `native/ft8_lib_build/patched/ft8/decode.c` per
...
- [ ] 1.2 Confirm `ft8_extract_likelihood()`'s own body is byte-for-byte unchanged (diff against
...
- [ ] 2.1 Add `int ft8_extract_llrs_at(const float* pcm, int pcm_len, float freq_hz, float
...
- [ ] 2.2 Implement it in `ft8_shim.c` per design.md D2's sketch: build the waterfall exactly as
...
- [ ] 2.3 Unit-check the inverse-mapping arithmetic in isolation before wiring it to the full
...
- [ ] 2.4 Round-trip sanity check against a REAL candidate (design.md Risks, last bullet): take any
...
- [ ] 3.1 Add `/EXPORT:ft8_extract_llrs_at` to `native/ft8_lib_build/rebuild_shim.bat`'s link step
      (design.md D4) — confirm no other line in that export list changes.
- [ ] 3.2 Bump `FT8_SHIM_VERSION` in `ft8_shim.h` from `20260041` to `20260042`, and
      `ExpectedShimVersion` in `src/OpenWSFZ.Ft8/Interop/Ft8LibInterop.cs:281` to match.
- [ ] 3.3 Rebuild Windows x64 (`rebuild_shim.bat`); record the new DLL's SHA256. Confirm via
...
- [ ] 3.4 Confirm CI (`.github/workflows/ci.yml`) rebuilds Linux x64 and macOS ARM64 from the same
...
- [ ] 5.1 Add a native or Python smoke test (design.md Open Questions — either satisfies this)
...
- [ ] 5.2 `dotnet build`: 0 warnings. `dotnet test`: full suite green — this change adds no C# code
...
- [ ] 6.1 Write the QA→Architect report: new DLL SHA256 (Windows; Linux/macOS if task 3.4 rebuilt
...
- [ ] 6.2 State plainly whether N1's harness (§4-§5 of the N1 spec: population, pairing, the gate,
