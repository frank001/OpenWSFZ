...
- `{dt,5:F1}` — timing offset in seconds, right-justified in a 5-character field with 1 decimal place (e.g. `  0.2`).
- `{freq,4}` — audio frequency offset in Hz, right-justified in a 4-character field (e.g. `2252`).
- `{message}` — decoded message text.

Each line SHALL be terminated with a newline (`\n`). The file SHALL be opened in append mode for each cycle and closed after writing, so that lines are visible to external readers between cycles.
...
- **WHEN** a decode result with `Snr = 3`, `Dt = 0.2`, `FreqHz = 2252`, and `Message = "Q4DSA QD1BER JO22"` is written with `dialFrequencyMHz = 7.074` at cycle time `17:29:30` on 2026-05-28
- **THEN** the written line SHALL be exactly `260528_172930     7.074 Rx FT8      3   0.2 2252 Q4DSA QD1BER JO22`
  (note: 3 spaces before "0.2" = 1-char field separator + 2-char left-pad of "0.2" in a 5-char dt field)
