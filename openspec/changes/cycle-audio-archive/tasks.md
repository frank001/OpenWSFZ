...
- [x] 9.3 Compare per-cycle decode yield between OpenWSFZ's own capture and WSJT-X's capture of the
      ...
      **Second addendum (2026-07-25 21:45, corrected 2026-07-26):** raw-waveform cross-correlation
      of the same 68 pairs (no decoder involved at all — PCM samples only). The original 21:45
      note (`qa/cycleframer-alignment-replay/2026-07-25-2145-raw-audio-crosscorrelation-check.md`,
      now marked superseded) used a ±50 ms search window and read the resulting truncation as a
      physical result: it reported only 11/68 pairs locking, on invariant lag ≡4 (mod 12) — a
      "fixed 0.333 ms residual" — with the other 57/68 called inconclusive. **That was wrong.**
      With the search widened (true lags run to −504 ms, 10× the original window), **all 68/68
      pairs lock**, at correlation 0.933–0.992, on the real invariant **lag ≡76 (mod 120) samples
      — a 10 ms grid matching the WASAPI shared-mode device period**, holding across every pair
      including lags out to 6044 samples; mod-12 = 4 is a weak corollary of that grid, not an
      independent finding. This corroborates capture-chain parity far more strongly than the
      original note claimed, on a second, independent line of evidence (raw signal alignment, not
      decoded message counts), and specifically rules out an accumulating clock-rate error in
      `WasapiAudioSource`'s resampling path over this session's ~21-minute span. No band-limited
      coherence refinement is needed — the corrected search already resolves all 68 pairs. Full
      corrected measurement and root-cause analysis:
      `qa/cycleframer-alignment-replay/2026-07-25-2300-alignment-root-cause.md`.
