...
- [ ] 1.1 Confirm `NTOKENS = 2,063,592`, `MAX22 = 4,194,304`, and the special-token assignments
...
- [ ] 1.2 Confirm the `ihashcall` formula (`n8 = 38*n8 + char_index` over up to 11 characters,
...
- [ ] 2.1 Implement the `ihashcall` formula in C# (new private helper in
...
- [ ] 2.2 Add unit tests against the project's known-vector table for `ihashcall` (the same
...
- [ ] 2.3 Cross-check a handful of values against `qa/rr-study/synth/packing.py`'s `ihashcall`
...
- [ ] 3.1 Add special-token packing (`"CQ"`, `"DE"`, `"QRZ"`, `"CQ nnn"` 3-digit numeral) to
...
- [ ] 3.2 Add nonstandard/compound-callsign packing to `Pack28`: for input that does not match
...
- [ ] 3.3 Confirm `Pack28` still returns an empty array unchanged for: directed CQ with a
...
- [ ] 3.4 Add a unit test confirming the existing standard-basecall path's output is byte-for-byte
...
- [ ] 4.1 Confirm `QsoAnswererService.ApplyApConstraints()` (~line 1161) requires no code change
...
- [ ] 4.2 Confirm `QsoCallerService`'s equivalent (~line 873) likewise requires no code change
      beyond the packer extension.
- [ ] 4.3 Add/extend integration tests for both services confirming AP constraints arm
...
- [ ] 5.1 Add a test proving AP-assisted decode succeeds for a nonstandard-callsign QSO exchange
...
- [ ] 7.1 Run the existing `OpenWSFZ.Ft8.Tests` and `OpenWSFZ.Daemon.Tests` suites to confirm no
      regression in existing standard-callsign AP-decode-assist behaviour or QSO
      caller/answerer state-machine tests.
- [ ] 7.2 Confirm this change requires no R&R study re-run: the existing S1–S8 synthetic corpus
