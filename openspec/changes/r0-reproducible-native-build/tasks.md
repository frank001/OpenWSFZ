...
- [ ] 1.1 Trace the include graph from the nine untracked linked objects and confirm the file list
...
- [ ] 1.2 Copy the confirmed file list from `C:\Temp\ft8_lib_headers` into `native/ft8_lib_vendor/`
...
- [ ] 1.3 Exclude `ft4_ft8_public/`, `demo/`, `test/`, `utils/`, `Makefile`, `README.md`,
      `.clang-format`, `.gitignore`, `.git/` from the vendored copy.
- [ ] 1.4 Record provenance mechanically: upstream remote URL, HEAD SHA
      ...
      output.
- [ ] 1.5 Decide and record where the already-tracked, MSVC-patched `decode.c`/`monitor.c`
...
- [ ] 1.6 Report the total committed size of the vendored tree.

## 2. Extend the rebuild script (D2)

- [ ] 2.1 Add `cl` compile steps to `native/ft8_lib_build/rebuild_shim.bat` for the nine
...
- [ ] 2.2 Confirm the `link` step's `/EXPORT:` list is unchanged (eleven symbols) and every object
      it references was produced earlier in the same script invocation.
- [ ] 2.3 Run a clean build from an empty `obj/` directory. If any vendored source file needs an
...
- [ ] 3.1 Replay ≥200 contiguous cycles from the pinned 20m corpus
      ...
      cycle range used.
- [ ] 3.2 Mechanically diff the two runs' serialised decode output (never an eyeballed summary or a
      count-of-decodes match). Report the diff command and its exit status.
- [ ] 3.3 **Evaluate the result honestly against both pre-registered branches:** PASS (zero
...
- [ ] 3.4 Run two independent clean builds (each from an empty `obj/` directory) and mechanically
...
- [ ] 3.5 Bump `FT8_SHIM_VERSION` (`src/OpenWSFZ.Ft8/Native/ft8_shim.h`) and `ExpectedShimVersion`
...
- [ ] 4.1 Write `THIRD-PARTY-NOTICES.md` at the repo root: MIT text with Kārlis Goba's copyright,
...
- [ ] 4.2 Scan the vendored tree for `GNU General Public`, `GPL`, `Affero`. Report every hit with
...
- [ ] 4.3 Confirm every vendored source file's own licence header is present and unmodified.
- [ ] 4.4 Confirm `ft4_ft8_public/` is absent from the vendored tree (should already hold from 1.3).

## 5. Replay-harness determinism and pinning (D3, D4)

- [ ] 5.1 Fix `qa/cycleframer-alignment-replay/p23_common.py:182`'s hash-randomised set iteration
      ...
      deterministic.
- [ ] 5.2 Check whether `p23_common.py` or `g2_verification_replay.py` already has a DLL-SHA-pinning
...
- [ ] 5.3 Wire the new pinning capability into the AC-1/AC-2 replay itself so those two runs
      self-verify which binaries they actually compared.

## 6. Wrap-up

- [ ] 6.1 Build clean (0 warnings), full `OpenWSFZ.Ft8.Tests` suite green (baseline: 306/306 on the
      G2(a) build — re-verify, don't assume it stays 306 after this change).
- [ ] 6.2 Write the QA→Architect report per the proposal's reporting requirements: upstream remote +
