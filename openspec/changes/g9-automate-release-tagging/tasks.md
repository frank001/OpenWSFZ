...
- [ ] 4.1 Open the PR and confirm the existing three-OS `build-test` matrix and gate
      `version-governance` (this PR's own diff touches no `VERSION` line, so G9b should report "no
      version bump required") both still pass.
- [ ] 4.2 After merge, confirm on GitHub that the `tag-release-version` job ran on the resulting
      ...
      `git ls-remote --tags origin` or the GitHub UI.
- [ ] 4.3 Confirm the tag push did not spawn a second `build-test` run (per task 2.2) — check the
      Actions run list for the repository around the merge time.
