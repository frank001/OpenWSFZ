...
- [x] 1.1 Add the new fixed 4,096-row state array, alongside the existing per-code table
      ...
      ```
- [x] 1.2 Add the new emission-site branch as an `else if` on the **same guard family** as the
      ...
      ```
- [x] 1.3 Add the new getter, immediately after `ft8_get_h12_by_code`:
      ```c
      ...
      ```
- [x] 1.4 Add the header declaration (`ft8_shim.h`, immediately after `ft8_get_h12_by_code`'s own
      declaration), doc comment matching that block's own style.
- [x] 1.5 Confirm by inspection that `hash_table_lookup`, `hash_table_add`, `announce_stamp`,
...
- [x] 2.1 `FT8_SHIM_VERSION` `20260049` → **`20260050`** (this exact number is pre-registered as a
...
- [x] 3.1 Windows: add `/EXPORT:ft8_get_h12_unresolved_by_code ^` to `rebuild_shim.bat`'s explicit
      export list (pattern: the `ft8_get_h12_by_code` entry already there). Linux: no change
      (default visibility).
- [x] 3.2 Rebuild every platform binary you have toolchain access to (Windows x64, Linux x64
...
- [x] 3.3 macOS ARM64 — deferred to CI's `macos-latest` leg, per standing project policy (not a
...
- [x] 4.1 `ExpectedShimVersion` `20260049` → `20260050` in `Ft8LibInterop.cs` — the **only**
      managed-code change this task list requires.
- [x] 4.2 Confirm, by diff review (not by running tests), that `IFt8NativeInterop.cs`,
...
- [x] 6.1 🔴 **Decode a small existing corpus (any already-captured leg is fine) on both the
