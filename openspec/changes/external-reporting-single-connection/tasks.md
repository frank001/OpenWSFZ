...
- [ ] 1.1 Add `Role` (string enum `"leader"`|`"follower"`, default `"leader"`), `LeaderUrl` (nullable
...
- [ ] 1.2 `WebApp.cs`'s `POST /api/v1/config` handler: extend the existing raw-JSON key-presence guard
...
- [ ] 1.3 `POST /api/v1/config` validation: reject (`HTTP 400`, no partial persistence) `role:
      "follower"` with a missing/empty `leaderUrl`, matching the existing out-of-range-port
      validation-error pattern.
- [ ] 1.4 `OpenWSFZ.Config.Tests`/`OpenWSFZ.Web.Tests`: missing-key-defaults test, round-trip test, the
...
- [ ] 2.1 In `ExternalReportingService`, branch every existing outbound send call site
...
- [ ] 2.2 Group one decode cycle's Status (if changed/due) and Decode datagrams into a single relay
...
- [ ] 2.3 `RelayToLeaderAsync`: `POST` the batch (base64-encoded datagram bytes, in order) to
...
- [ ] 2.4 Degrade-and-reconcile: on relay failure (connect error, timeout, non-2xx), send the same
...
- [ ] 2.5 `OpenWSFZ.Daemon.Tests`: relay batch is well-formed and ordered (assert against a fake HTTP
...
- [ ] 3.1 Add `POST /api/v1/external-reporting/relay` in `WebApp.cs`: accepts `{ followerInstanceId,
...
- [ ] 3.2 Add a single-consumer dispatch queue in `ExternalReportingService` that both the leader's own
...
- [ ] 3.3 `OpenWSFZ.Daemon.Tests`/`OpenWSFZ.Web.Tests`: relay endpoint dispatches a batch's datagrams in
...
- [ ] 4.1 Extend the leader's existing `HandleHaltTxAsync` to also `POST /api/v1/tx/abort` (the
      ...
      other follower.
- [ ] 4.2 `OpenWSFZ.Daemon.Tests`: Halt Tx aborts the leader and every reachable configured follower;
...
- [ ] 5.1 In the leader's existing Reply handler, when `TryExtractCallsign` succeeds but the callsign
...
- [ ] 5.2 `OpenWSFZ.Daemon.Tests`: the new distinct log entry fires for a callsign absent from the
      leader's own decode batch, and the pre-existing "no callsign extracted" log path is unchanged.

## 6. Documentation

- [ ] 6.1 Append new FR entries (FR-063 onward) to `REQUIREMENTS.md` §4.1 covering: the
      ...
      (`fix-external-reporting-appid-collision`).
- [ ] 6.2 Update the `REQUIREMENTS.md` §4.3 Integrations table row for "GridTracker2 / WSJT-X UDP
      protocol" to note multi-instance leader/follower relay support.
- [ ] 6.3 Add a `REQUIREMENTS.md` §10 revision-history row documenting this change, matching the
...
- [ ] 6.4 Ensure every new xUnit test's `DisplayName` carries its requirement-ID prefix (traceability
      gate G3); re-run `tools/TraceabilityCheck` and confirm `PASS: all requirements are mapped and
      all references are valid.`

## 7. Verification

- [ ] 7.1 Run the full existing test suite (`dotnet test` across all projects) and confirm every
...
- [ ] 7.2 Run `openspec validate --strict --all` and confirm this change's delta specs archive cleanly
