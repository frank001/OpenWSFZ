...
- [x] 1.1 Create branch `feat/siggen` from `main`

## 2. Scaffold `siggen.py` — argument parser and entry point

- [x] 2.1 Create `qa/rr-study/siggen.py` with a `main()` and `argparse.ArgumentParser`:
...
- [x] 2.2 Implement the two top-level dispatch paths in `main()`: single-scene mode
...
- [x] 3.1 Implement `parse_scene(path: str) -> tuple[dict, list[dict]]` that reads the
  ...
  signals)`
- [x] 3.2 Implement `resolve_config(scene_config: dict, args: argparse.Namespace) ->
...
- [x] 3.3 Implement `_parse_amplitude(d: dict) -> float` helper: returns `d["amplitude"]`
...
- [x] 4.1 Implement `render_sine`: generate a sinusoid using `np.sin(2π·freq·t + phase)`
  scaled by amplitude, placed in a zero-filled slot buffer at `start_s`
- [x] 4.2 Implement `render_square`: additive synthesis of odd harmonics
...
- [x] 4.3 Implement `render_sawtooth`: additive synthesis of harmonics
  (`scipy.signal.sawtooth` with `width=1`), scaled by amplitude, placed at `start_s`
- [x] 4.4 Implement `render_triangle`: additive synthesis of odd harmonics with
  alternating signs (`scipy.signal.sawtooth` with `width=0.5`), scaled by amplitude,
  placed at `start_s`
- [x] 4.5 Implement `render_chirp`: `scipy.signal.chirp(t, f0, t1, f1, method=method)`
...
- [x] 4.6 Implement `render_noise`: generate `np.random.default_rng(seed).standard_normal
...
- [x] 5.1 Implement `render_ft8(d: dict, n_samples: int, sample_rate: int, scene_config:
  ...
  of `n_samples`
- [x] 5.2 Verify lazy import: add a comment assertion that `synth.encoder` does not
  appear in the module-level import block of `siggen.py`

## 6. Mix and output routing

- [x] 6.1 Implement `render_scene(signals: list[dict], config: dict) -> np.ndarray`:
...
- [x] 6.2 Implement `write_outputs(buffer: np.ndarray, config: dict) -> None`:
...
- [x] 6.3 Implement device selection helper `_select_device(substring: str) -> int`
...
- [x] 7.1 Implement `run_batch(batch_path: str, cli_overrides: dict) -> bool`: read the
  ...
  `False` if any failed
- [x] 7.2 Wire `run_batch` into `main()`: call it when `--batch` is supplied; exit with
  code 1 if `run_batch` returns `False`

## 8. Verification

- [x] 8.1 Verify single `sine` scene: `python siggen.py sine_test.jsonl --out sine.wav`
...
- [x] 8.2 Verify `chirp` scene: confirm frequency rises audibly from start to end of
  sweep
- [x] 8.3 Verify multi-signal scene with `noise` + `sine` + `ft8`: confirm the output
  WAV is decodable by WSJT-X / libft8 (the FT8 signal survives the mix)
- [x] 8.4 Verify FT8 lazy import: run a scene with only `sine` and `noise`; confirm no
...
- [x] 8.5 Verify batch mode: create a 3-item `jobs.json`; confirm all three WAV files
  are produced; introduce a deliberate error in item 2 and confirm items 1 and 3 still
  complete
- [x] 8.6 Verify device routing: run `python siggen.py scene.jsonl --device "CABLE Input"`
...
- [x] 9.1 Create `docs/siggen-reference.md` covering all sections required by the spec:
...
- [x] 9.2 Add a "General-purpose signal generator — siggen.py" section to
  `docs/rr-synth-cli-guide.md` with a two-sentence overview and a link to
  `docs/siggen-reference.md`

## 10. Commit and PR

- [x] 10.1 Commit `qa/rr-study/siggen.py`, `docs/siggen-reference.md`, and the updated
  `docs/rr-synth-cli-guide.md` with a descriptive message
- [x] 10.2 Open a PR targeting `main` referencing this OpenSpec change
