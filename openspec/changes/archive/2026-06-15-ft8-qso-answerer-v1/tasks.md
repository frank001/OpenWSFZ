...
- [x] 8.2 Run full test suite; confirm all existing tests pass plus new tests green (377 total as of task 8.8)
- [x] 8.3 Run loopback validation: WSJT-X sends CQ via VoiceMeeter → OpenWSFZ auto-answers → full 6-message exchange completes → ADIF.log entry written → WSJT-X logs the contact — confirmed by UAT session `artefacts/ft8-qso-answerer-v1_items/202606151810`; two complete Idle→TxAnswer→WaitReport→TxReport→WaitRr73→Tx73→QsoComplete cycles observed in log; three ADIF entries written; WSJT-X sent back signal reports confirming decode of OpenWSFZ transmissions
- [x] 8.4 Verify WSJT-X reports DT within ±0.5 s for OpenWSFZ transmissions — DT within FT8 decode tolerance confirmed (WSJT-X decoded all OpenWSFZ transmissions and responded correctly across both UAT sessions); ±0.5 s specific measurement not captured from WSJT-X display but exchanges completed without timing rejection
- [x] 8.5 Verify operator abort (`POST /api/v1/tx/abort`) stops TX mid-QSO and returns to Idle — confirmed by UAT session `artefacts/ft8-qso-answerer-v1_items/202606152348`; `POST /api/v1/tx/abort` during TxAnswer playback returned HTTP 200; `KeyDownAsync` returned normally (D-007 race scenario); `ThrowIfCancellationRequested` fired; "aborted to Idle (was: TxAnswer, partner: PD2FZ)" logged; service returned to Idle without spurious ADIF write
- [x] 8.6 Verify watchdog abort: configure `tx.watchdogMinutes = 1`, allow WSJT-X to go silent mid-QSO; confirm abort logged and no ADIF entry written — confirmed by UAT session `artefacts/ft8-qso-answerer-v1_items/202606152348` (watchdogMinutes=4); partner went silent at WaitRr73; retries cycled 1–8/15 with no watchdog-reset log entries (D-008 fix confirmed); watchdog fired at exactly t+4 min during retry 8 TX; "TX session cancelled during TX (state: WaitRr73)" logged; "aborted to Idle (was: WaitRr73, partner: PD2FZ)" logged; no ADIF entry written; retry count at abort: 8 of 15 maximum
- [x] 8.7 Update G3 traceability gate to include new spec IDs
- [x] 8.8 Live-run crash fix: guard against empty callsign/grid in `HandleIdleAsync`; add callsign + grid input fields to Settings UI; update `settings.js` to load/save/snapshot both fields (TX-D03 implemented; regression test added)
