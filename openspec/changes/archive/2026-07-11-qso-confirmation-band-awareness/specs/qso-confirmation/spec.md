...
**Reason**: Superseded by "Contact (callsign) matching includes portable-suffix variants,
band-aware" below — "Partner" is renamed to "Contact" (the term hams actually use) and the
matching result becomes tri-state (band-aware) rather than a plain boolean, per the Captain's
explicit direction to mirror WSJT-X/JTAlert-style worked-before-by-band indication.

**Migration**: No data migration — this is a rename plus a richer result type for the same
underlying "is this the same station" comparison logic (exact match, or either side a
portable-suffixed variant of the other). Existing `Call`-labelled payload/index internals become
`Contact`-labelled; existing portable-suffix matching semantics (`Q1TST` matches `Q1TST/P`, both
directions) are unchanged and carried forward unmodified into the new requirement below.

### Requirement: Country and Region matching, with Unknown and synthetic entries excluded

**Reason**: Superseded by "Country, Continent, CQ Zone, and ITU Zone matching, band-aware, with
Unknown and synthetic entries excluded" below — "Region" is renamed to "Continent" (resolves a
naming collision with the pre-existing, unrelated Region *display* column in `web-frontend`), CQ
Zone and ITU Zone are added as two new matching axes using data already resolved by
`region-lookup` but not previously indexed, and all four axes become tri-state (band-aware)
rather than plain booleans.

**Migration**: No data migration — this is a rename (Region→Continent) plus two new axes plus a
richer per-axis result type, built on top of the same underlying entity/continent resolution via
`ICallsignRegionStore.TryGetRegion` and the same Unknown/synthetic-exclusion posture, both
carried forward unmodified into the new requirement below.
