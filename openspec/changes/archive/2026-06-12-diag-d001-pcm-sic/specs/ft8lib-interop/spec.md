...
in the shim. The expected constant SHALL be **`20260008`** (H3 diagnostic: PCM-domain SIC using
heap-allocated residual buffer and CP-FSK synthesis; version 20260007 was the reverted
three-pass SIC; version 20260006 was D-002 SNR bias calibration). If the returned value does not
...
- **WHEN** `Ft8LibInterop` loads the platform-appropriate `libft8` binary compiled from the
  committed shim source at `FT8_SHIM_VERSION = 20260008`
...
The binaries SHALL be built from the **`FT8_SHIM_VERSION = 20260008`** shim (H3 diagnostic:
PCM-domain SIC with heap-allocated buffers and CP-FSK synthesis).
