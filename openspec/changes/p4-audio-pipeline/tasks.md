...
- [x] 15.4 Run the daemon (`dotnet run --project src/OpenWSFZ.Daemon`), open the Settings page — confirmed: 13 real audio devices returned by `GET /api/v1/audio/devices`. Also discovered and fixed a UI bug: settings.js was persisting `dev.name` but `WasapiAudioSource.GetDevice` requires the device ID (GUID); fixed to `dev.id`.
  - **R-1 post-fix note (2026-05-21):** A user running a stale `bin\Debug\` binary built before commit `42dcae1` hit `InvalidProgramException` in `MMDeviceEnumeratorComObject..ctor()` — the stale runtimeconfig had `BuiltInComInterop.IsSupported=false` baked in from an earlier `PublishAot=true` build. Fix in `42dcae1` is confirmed correct. Users who experience this must rebuild from clean: `dotnet clean -c Debug src/OpenWSFZ.Daemon && dotnet run --project src/OpenWSFZ.Daemon`. `dotnet run` triggers a fresh build, regenerates the runtimeconfig with `BuiltInComInterop.IsSupported=true`, and the Settings page populates with real devices.
...
- [x] 15.6 Publish AOT (`dotnet publish -c Release -r win-x64 src/OpenWSFZ.Daemon`); confirmed build succeeded. ILC warning confirmed: `MMDeviceEnumeratorComObject..ctor()` is a `[ComImport]` COM constructor that cannot execute under NativeAOT (`BuiltInComInterop.IsSupported=false`). This affects both the enumeration and capture paths. The AOT-published binary (`dotnet publish -r win-x64`) is intentionally unsupported for WASAPI. Development and testing use `dotnet run` (JIT, no RID) where COM interop is available. See A-1 in qa-review.md for the full limitation note.
