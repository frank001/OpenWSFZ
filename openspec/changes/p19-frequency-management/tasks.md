...
- [ ] 10.3 Manual smoke: start the daemon, open the browser, verify the Frequencies tab lists the 15 default FT8 entries; add and delete a row; save; verify `frequencies.json` is updated on disk
- [ ] 10.4 Manual smoke (CAT path): enable CAT in Settings; verify `#dial-freq` becomes a dropdown on the main page; select a different frequency; verify the rig tunes (or that `app.json` `dialFrequencyMHz` updates when CAT is disabled)
- [ ] 10.5 Commit, push, open PR to `main`, confirm CI green
