# callsign-structure-validation Specification

## Purpose

Specifies the ITU Radio Regulations Article 19 structural shape grammar used to validate
callsign-position tokens in decoded FT8 messages, replacing the length-only D9-R3 oversized-
callsign guard. Grammar rules — the digit-run maximum, the total base-callsign length ceiling,
the reserved/never-allocated prefix exclusion list, and the project's synthetic Q-prefix carve-out
(NFR-021) — are loaded from `callsign-grammar.json` rather than hard-coded, so a future ITU
reallocation or calibration adjustment does not require a code change.
