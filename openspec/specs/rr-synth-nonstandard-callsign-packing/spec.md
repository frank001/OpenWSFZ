# rr-synth-nonstandard-callsign-packing Specification

## Purpose
Specifies Type 4 (`i3=4`) nonstandard/compound-callsign message packing and 22-bit
`ihashcall` hash computation in the independent QA synth encoder
(`qa/rr-study/synth/packing.py`), plus the extension needed for the standard-message packer to
reference such a callsign's hash via the `c28` field. This gives the R&R study tooling the
ability to produce a genuine Type 4 announcement and a corresponding hash-reference message,
enabling the linked-cycle effectiveness scenario
(`rr-linked-cycle-effectiveness-scenario`) to measure the hashed-callsign resolution mechanism
(`f-001-hashed-callsign-resolution`) under realistic decode conditions. Per the project's
existing QA-synth attestation convention, this packing logic is independently implemented from
the published FT8 protocol description (Franke/Somerville/Taylor, QEX 2020) and is not ported or
transliterated from `ft8_lib`/`ft8_shim.c`.
