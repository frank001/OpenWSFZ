...
## Purpose
Specifies the multi-pass spectrogram-domain iterative subtraction capability introduced in p15. After the first decode pass, successfully decoded signals are suppressed in the waterfall and a second pass is performed on the residual to recover weaker co-channel signals. This capability is implemented in the native `ft8_shim.c` and is exposed to managed code via the `ft8lib-interop` layer.
