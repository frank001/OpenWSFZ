# daemon-status-visibility Specification

## Purpose

This capability exposes the native FT8 decoder shim's actual loaded ABI version — checked
once at daemon startup by `Ft8LibInterop`'s self-test but previously discarded after the
comparison — on the status endpoint and in the settings UI, so an operator can diagnose a
shim mismatch without reading daemon logs.
