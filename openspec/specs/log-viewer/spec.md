...
This capability provides operator visibility into the daemon's own log output from the
browser: a polled tail of the last N lines on the settings page's Logs tab, and a
standalone full-log page for viewing the complete active log file. It reads whatever file
`LoggingPipeline` already produces — no change to log rotation, retention, or the on-disk
log format.
