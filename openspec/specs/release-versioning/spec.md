...
Specifies the project's canonical version source: the root `VERSION` file, its consumption by
`Directory.Build.props` and the assembly-version pipeline, the cross-surface consistency
invariant it anchors (build metadata, the daemon's status API and welcome banner, and the
`README.md`/`REQUIREMENTS.md` doc citations), the minor-version-per-user-facing-feature bump
rule, and the OpenSpec proposal `user_facing` marker convention that downstream tooling
(including CI gate G9, see the `ci-quality-gates` capability) relies on to classify changes.
