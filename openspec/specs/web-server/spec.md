...
The daemon SHALL expose `GET /api/v1/ws` that accepts WebSocket upgrade requests. On connect, the server SHALL immediately push one `status` JSON event. The server SHALL push a `heartbeat` event every 5 seconds while the connection is open. After each FT8 decode cycle the server SHALL push a `decode` event carrying the list of decoded results for that cycle (see `decode-events` spec for payload shape). The broadcast SHALL be fire-and-forget per connection; a connection that cannot accept a frame within 1 second SHALL be closed silently.
