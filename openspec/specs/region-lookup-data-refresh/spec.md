# region-lookup-data-refresh Specification

## Purpose

Specifies the operator-triggered mechanism for refreshing the `region-lookup` capability's
`callsign-regions.json` data from country-files.com's published country-file release, replacing
the daemon's active prefix→region table at runtime without a restart or automatic/background
fetching. Covers the refresh operation itself (fetch, convert, install), its failure handling
(a failed fetch or conversion SHALL leave the existing region data untouched), passing
individual-callsign exception entries through unfiltered (safe because `callsign-regions.json` is
never committed to version control), an operator-facing status view of the current table and the
most recent refresh attempt, and a diagnostic single-callsign lookup against the active table.
