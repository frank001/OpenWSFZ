...
(and its resolved DXCC entity/continent) ever logged to `ADIF.log` (`adif-log` capability),
built once at daemon startup and kept live as new QSOs are logged during the session. For each
decoded message, resolves three independent booleans — has this exact callsign (Partner), this
DXCC entity (Country), or this continent (Region) been worked before, anywhere in `ADIF.log`'s
history, on any band or mode — and attaches them to the decode-result WebSocket payload
alongside `region-lookup`'s existing `region` field. Fully advisory: a missing or unreadable
`ADIF.log`, a malformed line, or any resolution failure degrades to "not worked before" on the
affected axis and never withholds a decode from `ALL.TXT` or the UI, matching the
`region-lookup` capability's existing advisory-only guarantee.
