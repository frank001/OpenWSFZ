...
The method is private and is not called from anywhere in the production code path. It is also algorithmically wrong: it uses the old uniform base-37 decode (dividing by 37 at all six positions), while `DecodeCallsign28` now uses the correct mixed-radix decode (base-27 for positions 3–5, base-10 for position 2, base-37 for positions 0–1). For packed = 4 134 219 (K0ABC under mixed-radix), `IsValidCallsign28` decodes position 2 as `'6'` and would return incorrect results if ever connected.
