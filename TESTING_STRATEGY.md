...
* **Threading / async waits:** a test must never use a fixed-duration delay (`Task.Delay(150)`, `Task.Delay(TimeSpan.FromSeconds(1))`, `Thread.Sleep(...)`) as a synchronization barrier before an assertion &mdash; it passes on an idle machine and flakes under real CI load. Poll the actual condition instead, via the shared `OpenWSFZ.TestSupport.Poll` helper (`Poll.UntilAsync` and its typed wrappers `WaitForEqualAsync`/`WaitForCallAsync`/`WaitForCallCountAsync`), with a bounded timeout that fails loudly and states what it was waiting for. This is enforced by **Gate G10** (`tools/check_test_delay_sync.py`, run by `pre_merge_check.py` and CI): any new fixed-duration delay outside the pre-existing, individually justified exceptions in `openspec/changes/fix-flaky-test-delay-synchronization/test-delay-debt.md` is a hard CI failure. A delay is only ever acceptable when it isn't standing in for a poll at all &mdash; e.g. a mock's simulated I/O latency, or proving an *absence* (nothing exited/fired) where there is no positive condition to poll for; such cases still require a debt-file entry explaining why no poll applies.
