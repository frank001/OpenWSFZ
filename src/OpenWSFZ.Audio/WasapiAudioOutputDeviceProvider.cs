...
// cannot enumerate WASAPI devices. Development and testing use dotnet run (JIT, no RID),
// where COM interop is available. Resolution: replace [ComImport] usage with ComWrappers.

/// <summary>
/// Enumerates WASAPI render (output/playback) endpoints on Windows using NAudio's
...
/// Mirrors <see cref="WasapiAudioDeviceProvider"/> exactly, using <see cref="DataFlow.Render"/>
/// instead of <see cref="DataFlow.Capture"/>.
/// </summary>
[SupportedOSPlatform("windows")]
internal sealed class WasapiAudioOutputDeviceProvider : IAudioOutputDeviceProvider
{
    private readonly ILogger<WasapiAudioOutputDeviceProvider> _log;
    ...
    public WasapiAudioOutputDeviceProvider(ILogger<WasapiAudioOutputDeviceProvider> log)
        => _log = log;

    /// <summary>Test-only constructor — injects a delegate in place of the NAudio call.</summary>
    internal WasapiAudioOutputDeviceProvider(
        ILogger<WasapiAudioOutputDeviceProvider>  log,
    ...
    private IReadOnlyList<AudioDeviceInfo> EnumerateDevices()
    {
        ...
        try
        {
            ...
            // Windows Sound Settings still appear in the list.
            var endpoints = enumerator.EnumerateAudioEndPoints(
                DataFlow.Render, DeviceState.Active | DeviceState.Disabled);
        ...
        catch (Exception ex)
        {
            ...
            _log.LogWarning(ex,
                "WASAPI output device enumeration failed; returning {Count} device(s) collected before the error.",
                devices.Count);
        }
