...
The function is private and never called from any production path. It also uses a pure base-37 decode (dividing by 37 at all six positions), while the actual `DecodeCallsign28` now uses mixed-radix (base-27 for positions 3–5, base-10 for position 2, base-37 for positions 0–1). For packed = 4134219 (Q0ABC under mixed-radix), `IsValidCallsign28` decodes position 2 as `'6'` rather than `'0'` — the function would return incorrect results for virtually every callsign if it were ever connected.
