...
2b. 🔴 **The bare-literal audit (D12) is part of the task, not an extra.** A `#define K_*` search cannot see a bare literal (the OSD depth was invisible to it). Search `ft8_shim.c` **and** every patched `native/ft8_lib_build/patched/ft8/*.c` for numeric *tuning* literals on the decode path, using the `design.md` D12 definition (changes decode results without changing the protocol). **Protocol constants are ONLY:** the 6.25 Hz tone spacing, 79 / 174 / 91, Costas, 8 tones, 12 kHz. 🔴 **The 3.125 Hz sub-bin (`6.25 / K_FREQ_OSR`) and the half-symbol time sub-step (`symbol period / K_TIME_OSR`) are NOT protocol**: they are the decoder's own oversampling choices, so a literal derived from them is a *derived value of a tabled constant* and is **listed as derived, never excluded as protocol** (Architect, spec §12). **Report every one.** Each is hoisted-and-tabled, **derived (from which tabled constant)**, or named on the page's **"Not included"** note (with file, line, value, reason). **Where you are unsure, list it with the doubt stated. Nothing is omitted silently.**
...
8. **The bare-literal audit (D12):** the full list of numeric tuning literals you found on the decode path (file, line, value), each marked *hoisted-and-tabled*, *derived from `K_…`* or *"Not included"* with its reason; the doubts stated. An empty list must be stated as empty.
