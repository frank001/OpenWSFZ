...
# Watches the 40m instance (physical radio, "Microphone (2- USB Audio CODEC )", dial 7.074 MHz).
# Adapted from the validated 2026-07-28 template (qa/endurance/2026-07-28-supervisor-40m-overnight.sh)
...
# 221240c). Self-contained win-x64 publish, running from D:\Projects\claude\OpenWSFZ-40m-capture\.
#
# Paired with WSJT-X on the same physical radio feed (Captain-managed, not touched here) and with
# the sibling 20m/80m instance below via external-reporting relay (leader/follower).
#
...
#
# Usage: RESTART_WAIT_SECS=<n> bash qa/endurance/2026-07-29-supervisor-40m-overnight.sh
# (defaults to 300s per prior precedent.)

set -u

CAPTURE_DIR="D:/Projects/claude/OpenWSFZ-40m-capture"
...
find_daemon_pid() {
    ...
    # instance, never the paired 20m/80m instance running from a different capture dir under the
    # same image name.
    powershell.exe -NoProfile -Command \
        "(Get-CimInstance Win32_Process -Filter \"Name='OpenWSFZ.Daemon.exe'\" | Where-Object { \$_.CommandLine -like '*40m-capture*' } | Select-Object -First 1 -ExpandProperty ProcessId)" \
        2>/dev/null | tr -d '\r'
}
...
kill_and_restart() {
    local reason="$1"
    log_event "SUPERVISOR(40m): failure detected ($reason). Killing daemon and restarting (attempt $((retry_count+1))/$MAX_RETRIES)."
    ...
    if [ -n "$pid" ]; then
        ...
        if [ -n "$still_there" ]; then
            log_event "SUPERVISOR(40m): kill of PID $pid did NOT take (rc=$kill_rc, still running as $still_there) -- aborting this restart attempt."
            return 1
        fi
        log_event "SUPERVISOR(40m): killed PID $pid, confirmed dead."
    else
        log_event "SUPERVISOR(40m): no running OpenWSFZ.Daemon.exe (40m) PID found (already dead)."
    fi

    log_event "SUPERVISOR(40m): waiting ${RESTART_WAIT_SECS}s before restart."
    ...
    log_event "SUPERVISOR(40m): relaunch command issued ($EXE)."
    ...
    while [ "$waited" -lt 60 ]; do
        ...
        if [ -n "$newlog" ] && grep -q "CycleFramer started" "$newlog" 2>/dev/null; then
            log_event "SUPERVISOR(40m): restart CONFIRMED healthy -- new log $newlog, decode resumed."
        ...
        if [ -n "$newlog" ] && grep -q "Hosting failed to start\|address already in use" "$newlog" 2>/dev/null; then
            log_event "SUPERVISOR(40m): new instance failed to bind (port still held?) -- $newlog"
            return 1
        fi
        sleep 5
        waited=$((waited + 5))
    ...
    log_event "SUPERVISOR(40m): restart did NOT confirm healthy decode resume within 60s (no 'CycleFramer started' seen in any new log since $EXE was relaunched)."
    return 1
}

CURRENT_LOG=$(find_latest_log)
log_event "SUPERVISOR ARMED (40m, overnight corpus + build-reliability soak): watching $CURRENT_LOG. max_retries=$MAX_RETRIES restart_wait_secs=$RESTART_WAIT_SECS"

while true; do
    [ -z "$CURRENT_LOG" ] && CURRENT_LOG=$(find_latest_log)
    log_event "SUPERVISOR(40m): watch phase on $CURRENT_LOG"
    ...
    while true; do
        ...
        if [ -n "$latest_log" ] && [ "$latest_log" != "$CURRENT_LOG" ]; then
            log_event "SUPERVISOR(40m): log rotation detected -- switching watch from $CURRENT_LOG to $latest_log (no restart needed)."
            exec 3<&- 2>/dev/null
        ...
        if [ $((now - last_status_epoch)) -ge 3600 ]; then
            echo "DIGEST(40m) ($(date -u +%H:%M:%SZ)): retries_so_far=$retry_count"
    ...
    if [ "$retry_count" -ge "$MAX_RETRIES" ]; then
        log_event "SUPERVISOR(40m): failure ($failure_reason) but already at $MAX_RETRIES/$MAX_RETRIES retries -- giving up, no further restart attempts. Manual intervention needed."
        break
    fi
    ...
    if kill_and_restart "$failure_reason"; then
        ...
        if [ "$retry_count" -ge "$MAX_RETRIES" ]; then
            log_event "SUPERVISOR(40m): restart attempt $retry_count/$MAX_RETRIES failed to confirm healthy -- max retries reached, giving up."
            break
        fi
        log_event "SUPERVISOR(40m): restart attempt $retry_count/$MAX_RETRIES did not confirm healthy within 60s -- will re-enter watch loop and retry again on next failure."
        CURRENT_LOG=$(find_latest_log)
        continue
    fi
done

log_event "SUPERVISOR(40m) EXITED (retry_count=$retry_count)."
