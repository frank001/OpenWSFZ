...
# brief-multiday-20m-live-run.md). Watches the 8081 instance: SDR Uno -> Voicemeeter Out B1.
# Free to band-hop (brief item 4) -- but full recording stays on throughout: mode=all archives
# every cycle's WAV in addition to ALL.TXT decodes, regardless of band. This is free corpus, not
# just retune-timestamp logging -- there is no reason to run it any less completely than 8080
# just because it isn't the decisive corpus. Currently co-located with the 8080/radio/WSJT-X
# instance on 14.074 MHz (SDR Uno tuned there at session start) -- while that holds, this is the
# same-band, same-time, two-receiver overlap window the brief calls a genuine bonus (item 4). Log
# the retune when it happens (or rely on the >=5 min gap detection in cycle-archive.csv).
#
...
#
# Adapted from the validated 2026-07-29 template (qa/endurance/2026-07-29-supervisor-20m-capture-
...
#
# Usage: RESTART_WAIT_SECS=<n> bash qa/endurance/2026-07-31-supervisor-8081.sh
...
# Mirror of the 8080 supervisor's draft block -- see that file for the full incident writeup. This
# instance checks itself against 8080 as the sibling, same heuristic, same reasoning. Gated OFF
# (ENABLE_CROSS_INSTANCE_DECODE_CHECK=0) until reviewed -- tonight's incident only hit 8080, but
# the detection is symmetric since either instance could in principle be the one that degrades.
ENABLE_CROSS_INSTANCE_DECODE_CHECK="${ENABLE_CROSS_INSTANCE_DECODE_CHECK:-0}"
SIBLING_DIR="D:/Projects/claude/OpenWSFZ-8080-capture"
...
# to the last 4000 lines rather than grepping the whole file, since these logs run for many hours
# and grow large -- an unbounded grep here would get slower every time it's called over the life
# of a multi-day run.
recent_decode_mean() {
    local logfile="$1"
...
check_decode_collapse() {
    ...
    if [ "$verdict" = "bad" ]; then
        ...
        log_event "SUPERVISOR(8081): decode-collapse check: own_mean=$own_mean sibling_mean=$sibling_mean -- ONE-SIDED, streak=$_decode_check_bad_streak/$DECODE_CHECK_CONSECUTIVE"
        if [ "$_decode_check_bad_streak" -ge "$DECODE_CHECK_CONSECUTIVE" ]; then
            return 0
        fi
    else
        if [ "$_decode_check_bad_streak" -gt 0 ]; then
            log_event "SUPERVISOR(8081): decode-collapse check: own_mean=$own_mean sibling_mean=$sibling_mean -- recovered, resetting streak (was $_decode_check_bad_streak)"
        fi
        _decode_check_bad_streak=0
    fi
    return 1
...
find_daemon_pid() {
    ...
    # instance, never the paired 8080 instance running from a different capture dir under the
    # same image name.
...
kill_and_restart() {
    local reason="$1"
    log_event "SUPERVISOR(8081): failure detected ($reason). Killing daemon and restarting (attempt $((retry_count+1))/$MAX_RETRIES)."
    ...
    if [ -n "$pid" ]; then
        ...
        if [ -n "$still_there" ]; then
            log_event "SUPERVISOR(8081): kill of PID $pid did NOT take (rc=$kill_rc, still running as $still_there) -- aborting this restart attempt."
            return 1
        fi
        log_event "SUPERVISOR(8081): killed PID $pid, confirmed dead."
    else
        log_event "SUPERVISOR(8081): no running OpenWSFZ.Daemon.exe (8081) PID found (already dead)."
    fi

    log_event "SUPERVISOR(8081): waiting ${RESTART_WAIT_SECS}s before restart."
    ...
    log_event "SUPERVISOR(8081): relaunch command issued ($EXE)."
    ...
    while [ "$waited" -lt 60 ]; do
        ...
        if [ -n "$newlog" ] && grep -q "Heartbeat:" "$newlog" 2>/dev/null; then
            log_event "SUPERVISOR(8081): restart CONFIRMED healthy -- new log $newlog, heartbeat resumed."
        ...
        if [ -n "$newlog" ] && grep -q "Hosting failed to start\|address already in use" "$newlog" 2>/dev/null; then
            log_event "SUPERVISOR(8081): new instance failed to bind (port still held?) -- $newlog"
            return 1
        fi
        sleep 5
        waited=$((waited + 5))
    ...
    log_event "SUPERVISOR(8081): restart did NOT confirm healthy (no 'Heartbeat:' line seen in any new log within 60s since $EXE was relaunched)."
    return 1
}

CURRENT_LOG=$(find_latest_log)
log_event "SUPERVISOR ARMED (8081, free-hopping full record): watching $CURRENT_LOG. max_retries=$MAX_RETRIES restart_wait_secs=$RESTART_WAIT_SECS"

while true; do
    [ -z "$CURRENT_LOG" ] && CURRENT_LOG=$(find_latest_log)
    log_event "SUPERVISOR(8081): watch phase on $CURRENT_LOG"
    ...
    while true; do
        ...
        if [ -n "$latest_log" ] && [ "$latest_log" != "$CURRENT_LOG" ]; then
            log_event "SUPERVISOR(8081): log rotation detected -- switching watch from $CURRENT_LOG to $latest_log (no restart needed)."
            exec 3<&- 2>/dev/null
        ...
        if [ "$ENABLE_CROSS_INSTANCE_DECODE_CHECK" = "1" ] && [ $((now - _decode_check_last_epoch)) -ge "$DECODE_CHECK_INTERVAL_SECS" ]; then
            ...
            if check_decode_collapse; then
                failure_reason="decode collapse: own decode rate one-sidedly low vs. sibling 8080 for $DECODE_CHECK_CONSECUTIVE consecutive checks (~$((DECODE_CHECK_CONSECUTIVE * DECODE_CHECK_INTERVAL_SECS / 60)) min)"
                break
            fi
        fi

        if [ $((now - last_status_epoch)) -ge 3600 ]; then
            echo "DIGEST(8081) ($(date -u +%H:%M:%SZ)): retries_so_far=$retry_count"
    ...
    if [ "$retry_count" -ge "$MAX_RETRIES" ]; then
        log_event "SUPERVISOR(8081): failure ($failure_reason) but already at $MAX_RETRIES/$MAX_RETRIES retries -- giving up, no further restart attempts. Manual intervention needed."
        break
    fi
    ...
    if kill_and_restart "$failure_reason"; then
        ...
        if [ "$retry_count" -ge "$MAX_RETRIES" ]; then
            log_event "SUPERVISOR(8081): restart attempt $retry_count/$MAX_RETRIES failed to confirm healthy -- max retries reached, giving up."
            break
        fi
        log_event "SUPERVISOR(8081): restart attempt $retry_count/$MAX_RETRIES did not confirm healthy within 60s -- will re-enter watch loop and retry again on next failure."
        CURRENT_LOG=$(find_latest_log)
        continue
    fi
done

log_event "SUPERVISOR(8081) EXITED (retry_count=$retry_count)."
