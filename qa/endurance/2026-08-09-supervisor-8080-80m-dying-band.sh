#!/usr/bin/env bash
# Crash-recovery supervisor for the 2026-08-09 80m dying-band leg (density-floor corpus, port 8080).
#
# GOAL (HK-020: stated explicitly, not inherited): gather an OpenWSFZ ALL.TXT + WAV corpus on
# port 8080 ("Microphone (2- USB Audio CODEC )" -- the FT-991A's own audio path, 3.573 MHz/80m,
# radio retuned and dialFrequencyMHz corrected 18.100 -> 3.573 in config.json this session) to
# compare against the Captain's two live WSJT-X instances (WSJT-X - FT991A and
# WSJT-X - FT991A-Copy, both already decoding on the same antenna/band). Decode-only -- there is
# no TX/QSO purpose to this run. Purpose (Captain, 2026-08-09): 80m is expected to die as morning
# comes; this leg is opportunistic capture of the resulting natural decay in decode density down
# to a floor no prior corpus (20m min 15.3, 17m min 3/Q1 9.7) has reached. See the pre-registration
# for the read logic and void conditions: qa/cycleframer-alignment-replay/
# 2026-08-09-0149-qa-prereg-80m-dying-band-density-floor.md.
#
...
# does not support WASAPI). Reused, not rebuilt.
#
# PTT SAFETY: ptt.method is "AudioVox". AudioOnlyPttController (src/OpenWSFZ.Daemon/
...
# gate (this is decode-only). cat.enabled is false -- dialFrequencyMHz is therefore the ONLY band
# label in this instance's output; there is no mechanical way to re-verify it once armed beyond
# the pre-registration's ROW 0d spot-check of ALL.TXT's own frequency column.
#
...
# band changes -- this same OpenWSFZ-8080-capture/ dir has now carried 20m, 17m, and 80m corpora,
# each gathered and archived (_archived_20260808_{20m,17m}_already-in-artefacts/) before the next.
#
# This script's watch/kill/restart logic is UNCHANGED, line-for-line, from the HK-013-validated
# 2026-08-08-supervisor-8080-comparison-run.sh (live kill-tested that night) -- only header
# comments and log tags (8080-cmp -> 8080-80m) differ. Not re-run through a fresh live kill-test,
...
#
# Usage: RESTART_WAIT_SECS=<n> bash qa/endurance/2026-08-09-supervisor-8080-80m-dying-band.sh
...
kill_and_restart() {
    local reason="$1"
    log_event "SUPERVISOR(8080-80m): failure detected ($reason). Killing daemon and restarting (attempt $((retry_count+1))/$MAX_RETRIES)."
    ...
    if [ -n "$pid" ]; then
        ...
        if [ -n "$still_there" ]; then
            log_event "SUPERVISOR(8080-80m): kill of PID $pid did NOT take (rc=$kill_rc, still running as $still_there) -- aborting this restart attempt."
            return 1
        fi
        log_event "SUPERVISOR(8080-80m): killed PID $pid, confirmed dead."
    else
        log_event "SUPERVISOR(8080-80m): no running OpenWSFZ.Daemon.exe (8080) PID found (already dead)."
    fi

    log_event "SUPERVISOR(8080-80m): waiting ${RESTART_WAIT_SECS}s before restart."
    ...
    powershell.exe -NoProfile -Command \
        ...
        >/dev/null 2>&1
    log_event "SUPERVISOR(8080-80m): relaunch command issued ($EXE)."
    ...
    while [ "$waited" -lt 60 ]; do
        ...
        if [ -n "$newlog" ] && grep -q "Heartbeat:" "$newlog" 2>/dev/null; then
            log_event "SUPERVISOR(8080-80m): restart CONFIRMED healthy -- new log $newlog, heartbeat resumed."
        ...
        if [ -n "$newlog" ] && grep -q "Hosting failed to start\|address already in use\|Unhandled exception" "$newlog" 2>/dev/null; then
            log_event "SUPERVISOR(8080-80m): new instance failed to start -- $newlog"
            return 1
        fi
        sleep 5
        waited=$((waited + 5))
    ...
    log_event "SUPERVISOR(8080-80m): restart did NOT confirm healthy (no 'Heartbeat:' line seen in any new log within 60s since relaunch)."
    return 1
}
...
while true; do
    ...
    log_event "SUPERVISOR(8080-80m): watch phase on $CURRENT_LOG"
    ...
    while true; do
        if read -t 10 -r line <&3; then
            ...
            if [[ "$line" == *"Heartbeat:"*"=false"* ]]; then
                echo "HEARTBEAT-DROP(8080-80m): $line"
            fi
        ...
        if [ -n "$latest_log" ] && [ "$latest_log" != "$CURRENT_LOG" ]; then
            log_event "SUPERVISOR(8080-80m): log rotation detected -- switching watch from $CURRENT_LOG to $latest_log (no restart needed)."
            exec 3<&- 2>/dev/null
        ...
        if [ $((now - last_status_epoch)) -ge 3600 ]; then
            echo "DIGEST(8080-80m) ($(date -u +%H:%M:%SZ)): retries_so_far=$retry_count"
    ...
    if [ "$retry_count" -ge "$MAX_RETRIES" ]; then
        log_event "SUPERVISOR(8080-80m): failure ($failure_reason) but already at $MAX_RETRIES/$MAX_RETRIES retries -- giving up, no further restart attempts. Manual intervention needed."
        break
    fi
    ...
    if kill_and_restart "$failure_reason"; then
        ...
        if [ "$retry_count" -ge "$MAX_RETRIES" ]; then
            log_event "SUPERVISOR(8080-80m): restart attempt $retry_count/$MAX_RETRIES failed to confirm healthy -- max retries reached, giving up."
            break
        fi
        log_event "SUPERVISOR(8080-80m): restart attempt $retry_count/$MAX_RETRIES did not confirm healthy within 60s -- will re-enter watch loop and retry again on next failure."
        CURRENT_LOG=$(find_latest_log)
        continue
    fi
done

log_event "SUPERVISOR(8080-80m) EXITED (retry_count=$retry_count)."
