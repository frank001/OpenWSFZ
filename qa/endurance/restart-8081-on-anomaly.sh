...
PORT=8081
LOG="$CAPTURE_DIR/restart-supervisor.log"
...
log "QA-RESTART(8081): anomaly judged present >5min ($REASON). Executing bundled restart."

before_logs=$(snapshot_logs)
pid=$(find_pid)
if [ -z "$pid" ]; then
    log "QA-RESTART(8081): no running daemon found -- aborting, nothing to restart."
    exit 1
fi
log "QA-RESTART(8081): found daemon PID $pid."

taskkill //F //PID "$pid" >/dev/null 2>&1
sleep 2
still=$(find_pid)
if [ -n "$still" ]; then
    log "QA-RESTART(8081): kill did NOT take (still running as $still) -- aborting."
    exit 1
fi
log "QA-RESTART(8081): killed PID $pid, confirmed dead."

sleep 3
powershell.exe -NoProfile -Command \
    "Start-Process -FilePath '$EXE' -ArgumentList '--config','$CONFIG','--port','$PORT' -WorkingDirectory '$CAPTURE_DIR'" >/dev/null 2>&1
log "QA-RESTART(8081): relaunch command issued."
...
while [ "$waited" -lt 60 ]; do
    ...
    if [ -n "$newlog" ] && grep -q "Heartbeat:" "$newlog" 2>/dev/null; then
        log "QA-RESTART(8081): restart CONFIRMED healthy -- new log $newlog, heartbeat resumed."
        log "QA-RESTART(8081): *** RESULT: PASS ***"
        exit 0
    fi
    sleep 5
    waited=$((waited + 5))
...
log "QA-RESTART(8081): *** RESULT: FAIL -- did not confirm healthy within 60s. Escalate to the Captain, do not retry blindly. ***"
exit 1
