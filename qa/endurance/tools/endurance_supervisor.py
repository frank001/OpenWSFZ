...
**Canonical source, established 2026-09-25** (QA handoff task 1.6, capture-stall-detection-
unattended/capture-device-reresolution): every prior run copied this file into its own
artefacts/<run>/tools/ (gitignored), so there was no committed, diffable source anywhere --
finding the "current" version meant digging through the newest artefacts/ snapshot. This file
under qa/endurance/tools/ IS that source from now on; run_endurance.py-style launchers should
copy it FROM here, not author their own drift. Applied here at the same time (task 1.6, now
unblocked -- #187/#188 merged to main 9f54e6af 2026-09-24, live-verified 2026-09-25): the WINDOW
health loop now reads captureState/dataFlowing/lastChunkAgeMs (< 10000 ms) as its primary health
predicate instead of the old bare captureActive bool -- captureActive can stay true while the
daemon is itself mid-retry against a re-resolving or unavailable device, which is a coarser
signal than what's now available. captureRestartCount/watchdogRestartCount are read from every
poll and threaded into heartbeat.json and the problem/restart events, so a real endurance run's
own record shows the daemon's self-healing activity, not just the supervisor's own restart
counter. The independent newest-WAV-mtime check is UNCHANGED and still runs alongside the new
predicate, per HK-026: an instrument (here, the daemon's own status endpoint) cannot bound its
own blind spot, so a second, independent signal stays in place even though the new predicate is
...
def main():
    ...
    try:
        ...
        consec = 0; healthy_since = time.time(); unhealthy_since = None; stale_strikes = 0
        cyc_dir = arm.get("daemon", {}).get("config_cycle_audio_dir") or ""
        last_capture_restart_count = None; last_watchdog_restart_count = None
        while utcnow() < we:
            time.sleep(POLL_S)
            ...
            if not alive:
                ...
                try:
                    with urllib.request.urlopen("http://127.0.0.1:%s/api/v1/status" % port, timeout=5) as r:
                        s = json.loads(r.read().decode("utf-8"))
                    # QA handoff task 1.6 (post-#187/#188, 2026-09-25): the daemon's own
                    # captureState/dataFlowing/lastChunkAgeMs are a more precise health signal
                    # than the old captureActive bool alone -- captureActive can stay true while
                    # the daemon is itself mid-retry against a re-resolving or unavailable
                    # device. HK-026: this does NOT replace the independent newest-WAV-mtime
                    # check below -- an instrument cannot bound its own blind spot, and this
                    # predicate is still just the daemon's own self-report.
                    last_chunk_ms = s.get("lastChunkAgeMs")
                    healthy = (s.get("captureState") == "Capturing" and s.get("dataFlowing") is True
                               and last_chunk_ms is not None and last_chunk_ms < 10000)
                    if not healthy:
                        unhealthy_since = unhealthy_since or time.time()
                        if time.time() - unhealthy_since > 90:
                            problem = ("health predicate false for > 90 s (captureState=%s "
                                       "dataFlowing=%s lastChunkAgeMs=%s)" %
                                       (s.get("captureState"), s.get("dataFlowing"), last_chunk_ms))
                    else:
                        unhealthy_since = None
                    if not s.get("decodingEnabled"):
                        problem = "decodingEnabled false"
                    last_capture_restart_count = s.get("captureRestartCount")
                    last_watchdog_restart_count = s.get("watchdogRestartCount")
                except Exception as e:
            ...
            hb = {"t": iso(utcnow()), "phase": "WINDOW", "alive": alive, "problem": problem,
                  ...
                  "captureRestartCount": last_capture_restart_count,
                  "watchdogRestartCount": last_watchdog_restart_count}
            ...
            if problem:
                ...
                run.event("problem", what=problem, captureRestartCount=last_capture_restart_count,
                          watchdogRestartCount=last_watchdog_restart_count)
                ...
                consec += 1; stale_strikes = 0; unhealthy_since = None; healthy_since = time.time()
