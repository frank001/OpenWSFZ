#!/usr/bin/env python3
"""NFR-021 redaction pass -- full S1-S8 sweep, results/2026-09-12-4584900/.

Same method as redact_s1s8_20260912_decodes.py / redact_s1s8_20260907_decodes.py /
redact_s1s8_20260906_decodes.py / redact_s1s8_20260903_decodes.py / redact_row0k_decodes.py /
...
message_text column BEFORE rewriting, then do a byte-level rewrite (UTF-8-with-BOM, CRLF)
so line endings are not silently normalised.

This run is the first full S1-S8 sweep against `4584900d` (`main`/`qa/base` HEAD post-PR #168 --
the dev/nhard40-default-migration code merge, plus two test-only commits). Same calendar start
date (2026-09-12 UTC) as the prior sweep's run directory (`results/2026-09-12-fbf8c0b/`), hence
the `b` suffix on this run's directory name and script name, and the sha-based `45849` infix below
(a date-based infix would collide with that prior pass's `912`).

Usage:
    python redact_s1s8_20260912b_decodes.py
"""
...
_RUN_DIR = _HERE / "results" / "2026-09-12-4584900"
...
def main() -> int:
    ...
    for i, tok in enumerate(sorted_tokens, start=1):
        placeholder_of[tok] = f"<RDCT45849{i:02d}>"

    print(f"\n=== Assigning {len(placeholder_of)} placeholders ===")
    map_lines = [
        "# NFR-021 redaction map -- S1-S8 full sweep, 2026-09-12/13 (`4584900d`)",
        "",
        "Same method as `results/REDACTION-MAP.md` (ROW 0's own redaction pass) and the "
        "2026-09-03/06/07/12 sweeps' own maps -- imported from the project's own scanner "
        "(`qa/rr-study/nfr021_pre_merge_scan.py`), not reimplemented. Every token below is "
        "verified against this run's own `truth.csv` `message_text` column before rewriting "
        "(see `redact_s1s8_20260912b_decodes.py`'s guard step) -- none appear inside any "
        "injected truth message, so all are decoder output, never injected truth.",
        "",
        "Placeholders use a sha-based `45849` infix (`<RDCT45849nn>`), distinct from every prior "
        "pass's own infix (`RDCTnn`, `RDCTMnn`, `RDCTKnn`, `RDCTSnn`, `RDCT906nn`, `RDCT7nn`, "
        "`RDCT912nn`) so tokens from different redaction passes are never confused if files are "
        "read together. A date infix would have collided with the 2026-09-12 sweep's own "
        "`RDCT912` pass (same calendar start date, different run/sha) -- hence sha-based here. "
        "Deliberately NOT Q-prefix, for the same reason as the other maps: a Q-prefix placeholder "
        "would be indistinguishable from a legitimately injected synthetic call and would "
        "corrupt a future `truth.csv` join.",
        "",
