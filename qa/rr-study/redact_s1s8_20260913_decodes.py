#!/usr/bin/env python3
"""NFR-021 redaction pass -- full S1-S8 sweep, results/2026-09-13-4584900/.

Same method as redact_s1s8_20260912b_decodes.py / redact_s1s8_20260912_decodes.py / etc.
(RUNBOOK.md Sec.7.5): import the project's own scanner (qa/rr-study/nfr021_pre_merge_scan.py --
CALL_RE, classify(), scan(), fp()) rather than reimplementing it (HK-022), guard every flagged
token against this run's own truth.csv message_text column BEFORE rewriting, then do a byte-level
rewrite (UTF-8-with-BOM, CRLF) so line endings are not silently normalised.

This is sweep 2 of the Captain-directed six-sweep routine batch, same HEAD (`4584900d`) as sweep
1 -- a same-build, deterministic-seed repeat measurement.

Usage:
    python redact_s1s8_20260913_decodes.py
"""
...
_RUN_DIR = _HERE / "results" / "2026-09-13-4584900"
...
def main() -> int:
    ...
    map_lines = [
        ...
        "(see `redact_s1s8_20260913_decodes.py`'s guard step) -- none appear inside any injected "
        "truth message, so all are decoder output, never injected truth.",
        "",
