#!/usr/bin/env python3
"""NFR-021 redaction pass -- M1-M4 decode CSVs (m1m4_s5_decodes.csv, m3_s1_decodes.csv).

Same method as results/REDACTION-MAP.md's ROW 0 pass: import the project's own scanner
(qa/rr-study/nfr021_pre_merge_scan.py -- CALL_RE, classify(), fp()) rather than reimplementing it
(HK-022), guard that every flagged token is decoder output and not injected truth BEFORE
rewriting, then do a byte-level rewrite (UTF-8-with-BOM, CRLF) so line endings are not silently
normalised -- the standing trap flagged in this arm's own spec Section 6.

Usage:
    python redact_m1_m4_decodes.py
"""
...
_TARGET_FILES = ["m1m4_s5_decodes.csv", "m3_s1_decodes.csv"]
_TRUTH_FILES = [
    _HERE / "_work" / "m1m4_s5_truth" / "truth.csv",
    _HERE / "_work" / "m3_s1_truth" / "truth.csv",
]
_SCENARIO_FILES = list((_QA_ROOT / "scenarios").glob("*.json")) + \
                  list((_HERE / "scenarios").glob("*.json"))
...
def main() -> int:
    ...
    for i, tok in enumerate(sorted_tokens, start=1):
        placeholder_of[tok] = f"<RDCTM{i:02d}>"

    print(f"\n=== Assigning {len(placeholder_of)} placeholders ===")
    map_lines = [
        "# NFR-021 redaction map -- AWGN-FP M1-M4 result CSVs",
        "",
        "Same method as `results/REDACTION-MAP.md` (ROW 0's own redaction pass) -- imported from "
        "the project's own scanner (`qa/rr-study/nfr021_pre_merge_scan.py`), not reimplemented. "
        "Every token below is decoder output on a signal-free (S5) or signal-present (S1) offline "
        "replay, never injected truth -- verified against every render's own `truth.csv` "
        "`message_text` column before rewriting (see `redact_m1_m4_decodes.py`'s guard step).",
        "",
        "Placeholders use an `M` infix (`<RDCTMnn>`) distinct from ROW 0's own `<RDCTnn>` "
        "placeholders so the two passes' tokens are never confused if both CSVs are read "
        "together. Deliberately NOT Q-prefix, for the same reason as ROW 0's map: a Q-prefix "
        "placeholder would be indistinguishable from a legitimately injected synthetic call and "
        "would corrupt a future `truth.csv` join.",
        "",
    ...
    map_path = _RESULTS / "REDACTION-MAP-M1-M4.md"
    map_path.write_text("\n".join(map_lines) + "\n", encoding="utf-8")
    print(f"\nWrote {map_path}")
