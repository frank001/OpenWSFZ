...
**Added 2026-09-12, same-day, HK-022** — surgical, hand-added per the Architect's Ask A
(Captain-directed: "I did notice a FP that was not even reported at all."). Not produced by a
`--run-dir` regeneration (prohibited on a committed sweep dir; see Section 5); generated by
importing `_unexplained_decode_events`/`_unexplained_decodes_report_lines` from `analyse.py`
read-only against this run's own `truth.csv`/`wsjt-all.txt`/`owsfz-all.txt` and pasting the
output verbatim — the same numbers a `--run-dir` re-run would produce, without its side effects.
...
|---|---|---|---|---|
| **WSJT-X** | **all** | **0** | | |
| OpenWSFZ | S3 | 1 | 6.0 | 6.0 |
| OpenWSFZ | S4 | 1 | 9.2 | 9.2 |
| **OpenWSFZ** | **all** | **2** | | |
...
All nineteen runs that exercised the complete controlled battery (S1/S2/S3/S7 at minimum), oldest
first (**CORRECTED 2026-09-12, same day, HK-022: was "eighteen" before `4c7d5ad` was added back in
— see footnote ⁹**). `%GR&R` is each stage's own Summary-table figure (AIAG %Contribution, threshold ≤ 10% PASS). S5
