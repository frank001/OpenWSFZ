...
**Purpose of this run.** Sweep 6 — the **final** sweep of the Captain-directed six-sweep routine
batch — same HEAD as sweeps 1–5 (`4584900d`), a sixth same-build, deterministic-seed repeat
measurement. Run directory renamed from the harness's natural `2026-09-13-4584900` to
`2026-09-13-4584900-5` (sweeps 2/3/4/5 already hold the bare path, `-2`, `-3`, `-4`).

**Null hypotheses in scope** (unchanged from sweeps 2–5): H0(repeatability), H0(no
large/one-directional OpenWSFZ-only signal), H0(Gate A-W), H0(Gate A-Δ).

**What would constitute a meaningful result.** As the closing sweep of the batch, this run's job
is also to let Section 6 draw the batch's overall conclusion across all six same-build points,
not just this sweep in isolation.
...
| OpenWSFZ | +1.08 | -0.011 | 1.106 | 0.053 | PASS |
...
| Part-to-Part | 0.83 | 99.53% |
| Total GR&R | 0.00 | 0.47% |
| Total | 0.83 | 100.00% |
...
| Metric | Value | Verdict |
|---|---|---|
| %Contribution (GR&R) | 0.47% | PASS |
| %Tolerance (GR&R) | 93.87% | — |
| %Study Var (GR&R) | 6.87% | — |
| ndc | 20 | PASS |
...
**No gate failures across any of the six sweeps in this batch. No further investigation
required as a merge-blocking matter.**

- ✅ **Gate A-W** PASS (8/480 = 1.667%, 95% UB 2.987% ≤ 6%) and **Gate A-Δ** PASS (0/120 this
  sweep vs 8/360 rest-of-window, Fisher p = 1.0000) — identical to sweeps 1–5, a sixth
  consecutive 0/120 own-sweep AWGN reading on both appraisers. Not FRAGILE.
- ✅ **Final same-build repeatability tally (six sweeps).** Bit-identical on every single sweep:
  OpenWSFZ's S4/S5 pooled confusion matrix (TP=96, FN=12, κ=0.909), S7 P2 (WSJT-X 15/15, OpenWSFZ
  0/15), and S8 station F (OpenWSFZ 0/5). No cell in this battery was more reproducible.
- ℹ️ **A second OpenWSFZ-only marginal cell appears this sweep: S7 P1** (co_channel 2-stack,
  −5 dB, Δ13 Hz) reads OpenWSFZ 9/10 against a constant WSJT-X 10/10 — the first movement on this
...
- ℹ️ **S7 P3 and S8 station H remain the batch's WSJT-X-only jitter cells** (P3: WSJT-X 10, 10, 6,
  4, 10, 4 across the six sweeps, OpenWSFZ a constant 10/10 throughout; station H: WSJT-X 4, 0, 2,
  0, 2, 0, OpenWSFZ a constant 5/5 throughout) — the mirror image of the P0/P1 pattern, on the
  reference decoder instead.
- ℹ️ S1b, S7 "all", S8 remain within the batch's envelope across all six sweeps; no threshold
  crossed at any point in the batch.

## Section 6 — Historical trend: every full S1–S8 sweep to date

All twenty-five runs that exercised the complete controlled battery (S1/S2/S3/S7 at minimum),
oldest first. The last six rows (`4584900d #1`–`#6`) are the **same binary**, run back-to-back at
the Captain's direction as a same-build repeat-measurement batch — the first time this study has
had six independent capture passes of one unchanged build to draw on (footnote 12). `%GR&R` is
...
(14/480=2.917%, 95% UB 4.522%), FRAGILE. `4584900d #1`–`#6` — Gate A-W PASS (8/480=1.667%, 95% UB
2.987%, identical across all six sweeps since none contributed a new AWGN event), **not
...
`4584900d`'s own code path, so all six rows of this batch are already correct at source.
...
`4584900d #6` (this run): S7 173/215 (OpenWSFZ) vs 209/215 (WSJT-X); S8 55/60 both; pooled
228/264 = 86.36%. **INFO only.**
...
requiring a new commit between them. Sweeps sharing one SHA are labelled `<sha> #1`…`#6` in this
table, in sweep order. Where two sweeps in the batch would otherwise land on the same
`results/<date>-<sha7>/` git path, the later sweep's directory is renamed with a numeric suffix
(`-2` … `-5`) before committing, per this table's own header row (`4584900d`'s six directories are
`results/2026-09-12-4584900/`, `results/2026-09-13-4584900/`, and `results/2026-09-13-4584900-{2,
3,4,5}/`, sweeps #1–#6 respectively). The table's `#N` numbering and the directory suffix
numbering are independent (`#N` counts sweeps of one SHA regardless of date; the directory suffix
counts only same-day path collisions). A same-SHA group is a **genuine repeatability replicate**,
not an independent build — read the group as repeated Gage R&R "trial" observations of the
measurement system itself, not as separate code-verification points, and see each sweep's own
Section 1/5 for the batch's running analysis as it accumulated.

**Reading it — batch summary (six same-build sweeps, `4584900d #1`–`#6`):** twenty-five full
sweeps now, the last six a genuine same-build repeat series — the largest such series this study
has run. **What proved perfectly reproducible, all six times:** OpenWSFZ's S4/S5 pooled attribute
reading (TP=96/FN=12, κ=0.909 exactly), S7 P2 (WSJT-X 15/15, OpenWSFZ 0/15), S8 station F
(OpenWSFZ 0/5), and Gate A-W/Gate A-Δ (8/480=1.667%, not FRAGILE, since none of the six sweeps'
...
itself attributed there to a low WSJT-X reading, not an OpenWSFZ change). **Overall: six
consecutive PASS verdicts, zero gate failures, one non-blocking finding worth a defect record.**
