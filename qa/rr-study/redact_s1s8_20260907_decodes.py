#!/usr/bin/env python3
"""NFR-021 redaction pass -- full S1-S8 sweep, results/2026-09-07-4cc1984/.

Same method as redact_s1s8_20260903_decodes.py / redact_row0k_decodes.py /
redact_m1_m4_decodes.py / results/REDACTION-MAP.md's ROW 0 pass (RUNBOOK.md Sec.7.5): import the
project's own scanner (qa/rr-study/nfr021_pre_merge_scan.py -- CALL_RE, classify(), scan(), fp())
rather than reimplementing it (HK-022), guard every flagged token against this run's own
truth.csv message_text column BEFORE rewriting (only S5/S7 are expected to carry
noise-hallucinated callsign-shaped decoder output; every other scenario's flagged set is
expected empty), then do a byte-level rewrite (UTF-8-with-BOM, CRLF) so line endings are not
silently normalised.

This run is the first live exercise of the S5-GATE-SIZING Amendment 1 population (Gate A /
Check B scored separately) -- Gate A fired FAIL (3/120 AWGN slots, all three noise-floor
hallucinations at -26/-26/-27 dB, all in part 1). Those three decoder-output strings are
callsign-shaped but never injected -- exactly the RUNBOOK Sec.7.5 phenomenon -- and must not
reach git in raw form.

Usage:
    python redact_s1s8_20260907_decodes.py
"""
...
_RUN_DIR = _HERE / "results" / "2026-09-07-4cc1984"
...
def main() -> int:
    ...
    for i, tok in enumerate(sorted_tokens, start=1):
        placeholder_of[tok] = f"<RDCT7{i:02d}>"
    ...
    map_lines = [
        ...
        "`REDACTION-MAP-M1-M4.md`, `REDACTION-MAP-ROW0K.md`, and the 2026-09-03 sweep's own map "
        "-- imported from the project's own scanner (`qa/rr-study/nfr021_pre_merge_scan.py`), not "
        "reimplemented. Every token below is decoder output at S5 noise-floor SNR (RUNBOOK.md "
        "Sec.7.5's documented phenomenon: the FT8 CRC occasionally passes for a random bit "
        "pattern that happens to be callsign-shaped), never injected truth -- verified against "
        "this run's own `truth.csv` `message_text` column before rewriting (see "
        "`redact_s1s8_20260907_decodes.py`'s guard step). This run is the first live exercise of "
        "the S5-GATE-SIZING Amendment 1 Gate A / Check B population; all three flagged tokens "
        "fall inside Gate A's AWGN window (S5 parts 0/1) and were the cause of Gate A's FAIL "
        "verdict.",
        "",
        "Placeholders use a `7` infix (`<RDCT7nn>`) distinct from ROW 0's own `<RDCTnn>`, "
        "M1-M4's `<RDCTMnn>`, ROW 0k's `<RDCTKnn>`, and the 2026-09-03 sweep's `<RDCTSnn>` "
        "placeholders so tokens from different redaction passes are never confused if files are "
        "read together. Deliberately NOT Q-prefix, for the same reason as the other maps: a "
