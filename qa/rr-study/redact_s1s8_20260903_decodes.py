#!/usr/bin/env python3
"""NFR-021 redaction pass -- full S1-S8 sweep, results/2026-09-03-35378b9/.

Same method as redact_row0k_decodes.py / redact_m1_m4_decodes.py / results/REDACTION-MAP.md's
ROW 0 pass (RUNBOOK.md Sec.7.5): import the project's own scanner
...
callsign-shaped decoder output; every other scenario's flagged set is expected empty), then do a
byte-level rewrite (UTF-8-with-BOM, CRLF) so line endings are not silently normalised.

This run's raw logs and matched CSVs are one file per scenario/whole-run (not split into a
_work/ subtree like the AWGN-FP replay passes), so target files are listed explicitly below
rather than globbed -- new files added to the run dir later are NOT picked up automatically,
by design (a silent glob is how the *_matched.csv contamination this policy exists for happened
in the first place).

Usage:
    python redact_s1s8_20260903_decodes.py
"""
...
_RUN_DIR = _HERE / "results" / "2026-09-03-35378b9"
...
def main() -> int:
    ...
    for i, tok in enumerate(sorted_tokens, start=1):
        placeholder_of[tok] = f"<RDCTS{i:02d}>"
    ...
    map_lines = [
        ...
        "`REDACTION-MAP-M1-M4.md`, and `REDACTION-MAP-ROW0K.md` -- imported from the project's "
        "own scanner (`qa/rr-study/nfr021_pre_merge_scan.py`), not reimplemented. Every token "
        ...
        "`message_text` column before rewriting (see `redact_s1s8_20260903_decodes.py`'s guard "
        "step).",
        "",
        "Placeholders use an `S` infix (`<RDCTSnn>`) distinct from ROW 0's own `<RDCTnn>`, "
        "M1-M4's `<RDCTMnn>`, and ROW 0k's `<RDCTKnn>` placeholders so tokens from different "
        "redaction passes are never confused if files are read together. Deliberately NOT "
        "Q-prefix, for the same reason as the other maps: a Q-prefix placeholder would be "
        "indistinguishable from a legitimately injected synthetic call and would corrupt a "
        "future `truth.csv` join.",
        "",
