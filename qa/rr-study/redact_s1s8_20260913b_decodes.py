#!/usr/bin/env python3
"""NFR-021 redaction pass -- full S1-S8 sweep, results/2026-09-13-4584900-2/.

Same method as the prior sweeps' own redact_s1s8_*.py scripts (RUNBOOK.md Sec.7.5): import the
...
truth.csv message_text column BEFORE rewriting, then do a byte-level rewrite (UTF-8-with-BOM,
CRLF) so line endings are not silently normalised.

This is sweep 3 of the Captain-directed six-sweep routine batch, same HEAD (`4584900d`) as
sweeps 1-2 -- a same-build, deterministic-seed repeat measurement. Its run directory was renamed
from the harness's natural `2026-09-13-4584900` to `2026-09-13-4584900-2` (see Section 6 footnote
12 of sweep 2's report) because sweep 2 already committed that exact <date>-<sha7> path on its
own branch -- this avoids a future merge conflict when both branches land on main.

Usage:
    python redact_s1s8_20260913b_decodes.py
"""
...
_RUN_DIR = _HERE / "results" / "2026-09-13-4584900-2"
...
def main() -> int:
    ...
    for i, tok in enumerate(sorted_tokens, start=1):
        placeholder_of[tok] = f"<{_PLACEHOLDER_PREFIX}{i:02d}>"

    print(f"\n=== Assigning {len(placeholder_of)} placeholders ===")
    map_lines = [
        "# NFR-021 redaction map -- S1-S8 full sweep, 2026-09-13 (`4584900d`, sweep 3 of 6)",
        "",
        "Same method as prior sweeps' own maps -- imported from the project's own scanner "
        ...
        "(see `redact_s1s8_20260913b_decodes.py`'s guard step) -- none appear inside any "
        "injected truth message, so all are decoder output, never injected truth.",
        "",
        f"Placeholders use the `{_PLACEHOLDER_PREFIX}` infix, distinct from every prior pass's "
        "own infix. Deliberately NOT Q-prefix, for the same reason as the other maps: a "
