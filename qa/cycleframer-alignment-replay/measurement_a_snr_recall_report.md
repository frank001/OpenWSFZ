...
**Max separation across common-support bins: 36.0 points.** Full three-band density ordering (80m >= 10m >= 20m) holds in 15/26 bins with all three bands present (58%); fails -- curves cross -- in 11/26 (42%). This replaces a prior version of this check that tested only the outer band pair (80m vs 20m) and reported 26/26 = 100% regardless of 10m's position; see 2026-07-31-0029 S1 / dev-task 3 for why that was wrong.
...
**Mechanical outcome per the table above: ROWS 3 AND 4 BOTH FIRE (non-monotone in density, AND curves cross in 11/26 bins) -> the reversal is NOT licensed. The co-channel withdrawal is dead (row 1 excluded by the >=10pt separation), but this is not evidence of competition either -- it is a 20m-specific deficit of unknown mechanism. ESCALATE, do not interpret further, do not rationalise.**
