"""D-001 Isolated-Miss Pipeline Diagnosis -- live VB-CABLE replay driver, GENERIC corpus version.

Adapted from
`qa/rr-study/results/2026-07-23-d9ab692-d001-isolated-pipeline-diagnosis/run_isolated_replay.py`
(committed `0726bd6`, unmodified core logic -- daemon lifecycle, audio playback, cycle-block
parsing, and Gate R / classification are byte-identical). Disclosed changes:

1. **`--wav-dir` and `--samples` / `--out` are parameters**, not hardcoded to the 07-07 session.
   See `materialise_isolated_sample_generic.py`'s own header for why `<corpus>/wsjt-x/wav/` is
   the correct source directory for these two arm corpora.
2. **`--port` is a parameter** (still isolated, still never touches the operator's real config)
   so arm A and arm B runs never collide if run back-to-back without a clean shutdown gap.
3. Everything else -- boundary alignment, noise-gap disposable filler, flush-wait, cycle-block
   delimiter parsing, Gate R exact-message-plus-30Hz-tolerance match, CG-failure /
   LDPC-convergence-failure / Ambiguous classification -- is unmodified.

NFR-021: only ts/freq/snr/band/candidate-count/LLR values are ever written to the committed
result file. Message text is read into memory only, never written to any output.

Usage:
    python run_isolated_replay_generic.py --samples <isolated_sample_candidates_with_msg.json>
        --wav-dir <corpus>/wsjt-x/wav --out <arm_results.json> --port 8099
        [--target-per-stratum 20] [--max-tries-per-stratum 60]
"""
...
def fetch_full_log(base_url: str) -> str:
    r = requests.get(f"{base_url}/api/v1/logs/full", timeout=10)
    r.raise_for_status()
    return r.text


def fetch_all_txt(all_txt_path: Path) -> str:
    if not all_txt_path.exists():
        return ""
    return all_txt_path.read_text(encoding="utf-8", errors="replace")


def classify_reproduced_miss(debug_info: dict,
                              reference_low_llr_threshold: float | None) -> tuple[str, dict]:
    ...
    if total <= 3:
        if (mean_llr is not None and reference_low_llr_threshold is not None
                and mean_llr < reference_low_llr_threshold):
...
def replay_one_candidate(candidate: dict, wav_dir: Path, device_idx: int,
                          log_cursor: TextCursor, txt_cursor: TextCursor,
                          gap_seed: int, base_url: str, all_txt_path: Path) -> dict:
    wav_path = wav_dir / f"{candidate['ts']}.wav"
    ...
    log_text = fetch_full_log(base_url)
    txt_text = fetch_all_txt(all_txt_path)
...
def run_reference_pool(ref_samples_path: Path, wav_dir: Path, device_idx: int,
                        log_cursor: TextCursor, txt_cursor: TextCursor,
                        base_url: str, all_txt_path: Path) -> list[float]:
    ref_samples = json.loads(ref_samples_path.read_text(encoding="utf-8"))
    llr_values: list[float] = []
    seed_base = 900000
    for band in BANDS:
        for i, rec in enumerate(ref_samples.get(band, [])[:10]):
            result = replay_one_candidate(rec, wav_dir, device_idx, log_cursor, txt_cursor,
                                           seed_base + i, base_url, all_txt_path)
...
def main() -> None:
    ...
    ap.add_argument("--samples-dir", required=True, type=Path,
                    help="the --out-dir passed to materialise_isolated_sample_generic.py")
    ap.add_argument("--wav-dir", required=True, type=Path)
    ap.add_argument("--out", required=True, type=Path)
    ap.add_argument("--port", type=int, default=8099)
    ap.add_argument("--target-per-stratum", type=int, default=20)
    ap.add_argument("--max-tries-per-stratum", type=int, default=60)
    args = ap.parse_args()

    base_url = f"http://localhost:{args.port}"
    run_dir = args.samples_dir / "_work" / "daemon_run"
    all_txt_path = run_dir / "OpenWSFZ_ALL.TXT"

    samples_path = args.samples_dir / "_work" / "isolated_sample_candidates_with_msg.json"
    if not samples_path.exists():
        sys.exit(f"ERROR: {samples_path} not found -- run materialise_isolated_sample_generic.py first")
    samples = json.loads(samples_path.read_text(encoding="utf-8"))

    proc = start_daemon(run_dir, args.port)
    results: dict[str, dict] = {}
    try:
        wait_ready(base_url)
        configure_daemon(base_url, run_dir / "logs", all_txt_path)
        ...
        log_cursor.new_lines(fetch_full_log(base_url))
        txt_cursor.new_lines(fetch_all_txt(all_txt_path))
        ...
        for band in BANDS:
            ...
            log(f"\n=== Stratum: {band} -- target {args.target_per_stratum} reproduced, "
                f"up to {args.max_tries_per_stratum} tries ===")
            for i, cand in enumerate(candidates):
                if len(collected) >= args.target_per_stratum or tried >= args.max_tries_per_stratum:
                    break
                tried += 1
                r = replay_one_candidate(cand, args.wav_dir, device_idx, log_cursor, txt_cursor,
                                          gap_seed=100000 + i, base_url=base_url,
                                          all_txt_path=all_txt_path)
                if r["decoded_on_replay"]:
                    decoded_on_replay_count += 1
                    log(f"  [{tried}/{args.max_tries_per_stratum}] {cand['ts']} @ "
                        f"{cand['freq_hz']}Hz ({cand['wsjt_snr_db']} dB) -> "
                        f"DECODED-ON-REPLAY (excluded from split)")
                else:
                    ...
                    collected.append(r)
                    log(f"  [{tried}/{args.max_tries_per_stratum}] {cand['ts']} @ "
                        f"{cand['freq_hz']}Hz ({cand['wsjt_snr_db']} dB) -> reproduced, "
                        f"prelim={verdict}, total_candidates={debug_info['total_candidates']}, "
                        f"n_blocks_seen={r['n_blocks_seen']} "
                        f"({len(collected)}/{args.target_per_stratum} collected)")
        ...
        if need_reference:
            log("\n=== A reproduced miss had <=3 total candidates -- running the reference "
                "success-pool sample for the meanAbsLLR baseline ===")
            ref_path = args.samples_dir / "_work" / "reference_success_sample_with_msg.json"
            reference_llr_values = run_reference_pool(ref_path, args.wav_dir, device_idx,
                                                        log_cursor, txt_cursor, base_url,
                                                        all_txt_path)
        else:
            log("\nNo reproduced miss had <=3 total candidates -- skipping the reference-pool run.")
        ...
        log(f"\nResults (committed, no message text) written to {args.out}")
