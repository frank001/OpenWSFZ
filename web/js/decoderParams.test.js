 ...
 * shim 20260054, FR-077). Only the pure, exported functions are exercised here; the DOM rendering
...
test('FR-077: the kind constants are the API\'s wire values', () => {
  assert.equal(KIND_RUNTIME, 'runtime');
  assert.equal(KIND_COMPILE_TIME, 'compile-time');
});

test('FR-077: groupEntries splits into the two groups and keeps the native order within each', () => {
...
test('FR-077: groupEntries loses no entry (every input lands in exactly one group)', () => {
...
test('FR-077: groupEntries throws on an empty or missing table rather than showing nothing', () => {
...
test('FR-077: groupEntries throws on an unknown kind rather than silently dropping the row', () => {
...
test('FR-077: formatValue shows the API\'s number verbatim, with no rounding or re-formatting', () => {
...
test('FR-077: differsFromDefault is true only when the value is not the compiled default', () => {
...
test('FR-077: rowModel carries name, value, default and the differs flag as text/plain data', () => {
...
test('FR-077: a name that looks like markup is carried as data, never interpreted (rows are built with textContent)', () => {
