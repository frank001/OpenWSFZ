...
|---|---|---|---|
| `LW9END 9A7W R-11` | `val = 64 + (−11 + 35) = 88` | **88** | ✅ Yes |
| `XX9YYY 9A7W R+08` | `val = 64 + (8 + 35) = 107` | **107** | ✅ Yes |

The G6 fixture `260529_000030.expected.txt` lists `LW9END 9A7W R-11` as a mandatory decode. With `val ≤ 84`, the `IsValidExtra15` filter would reject val = 88, causing `TryUnpack` to return `null` for that message even if LDPC and CRC-14 both pass. Setting the ceiling to 84 therefore introduces a correctness regression for R-prefix QSO messages.
