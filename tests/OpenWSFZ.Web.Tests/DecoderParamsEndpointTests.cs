...
/// FR-069): the native decoder's parameter table, read-only.
/// <para>
...
public sealed class DecoderParamsEndpointTests : IClassFixture<WebTestFactory>
{
    ...
    [Fact(DisplayName = "FR-069: GET /api/v1/decoder/params returns 200 with shimVersion and every entry as name/kind/value/default, in order")]
    ...
    [Fact(DisplayName = "FR-069: the response carries the native decoder's values, NOT app.json's (osd_nhard_max 40 while the config says 55)")]
    ...
    [Fact(DisplayName = "FR-069: the provider is read on EVERY request and the result is never cached")]
    ...
    [Theory(DisplayName = "FR-069: every mutating verb on the route returns 405")]
    [InlineData("POST")]
    [InlineData("PUT")]
    [InlineData("PATCH")]
    [InlineData("DELETE")]
    ...
    [Fact(DisplayName = "FR-069: a provider that throws yields 503 with a message, never an empty table")]
    ...
    [Fact(DisplayName = "FR-069: no provider wired (a minimal fixture) yields 503, not an empty table")]
    ...
    [Fact(DisplayName = "FR-069: a provider that returns NO parameters yields 503, not an empty table")]
    ...
    [Fact(DisplayName = "FR-069: the route is behind the same authentication as the other /api/v1 routes (401 without the key, 200 with it)")]
    ...
    [Fact(DisplayName = "FR-069: through the composed daemon the endpoint serves the REAL native table (osd_nhard_max default 60) and the same shimVersion as /api/v1/status")]
