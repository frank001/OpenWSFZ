...
| Configuration              | Read on startup; written when the operator Saves from Settings, toggles the decode start/stop control, or changes the cycle-countdown visibility. | A configuration file at a default or overridden path. Fields include (non-exhaustive): audio device name, `DecodingEnabled`, `ShowCycleCountdown`. |
