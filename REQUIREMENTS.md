...
| NFR-024 | QA harness — compliant report generation | The QA harness analysis scripts (`analyse.py`, `analyse_corpus.py`, and any future equivalent) SHALL generate `report.md` in full NFR-023-compliant format directly and automatically, without requiring manual post-processing. A supplementary `report-v2.md` alongside a non-compliant `report.md` is not an acceptable substitute — the compliance obligation falls on the harness output, not on a hand-written document added afterwards. A `report.md` committed to `main` that does not satisfy all five NFR-023 mandatory sections is a **merge-blocking defect**, regardless of whether a `report-v2.md` exists alongside it. This requirement is in force immediately; existing non-compliant `report.md` files in historical result directories are grandfathered but SHALL NOT be used as precedent for new runs. | Must Have |
