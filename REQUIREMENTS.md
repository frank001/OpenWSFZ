...
| FR-019 | Configurable application logging                      | The application SHALL implement structured logging using the .NET `Microsoft.Extensions.Logging` infrastructure. Five log levels SHALL be supported, selectable from a dropdown on the Settings page and persisted to the configuration file (`AppConfig.LogLevel`, default: `"INFO"`): **OFF** (all output suppressed), **FATAL** (unrecoverable failures and unexpected exceptions only), **WARNING** (potential issues that do not halt operation, e.g. buffer overflows, device warnings), **INFO** (routine operational events: pipeline start/stop, device selection, decode cycle results, configuration changes), **DEBUG** (fine-grained diagnostic detail: sample counts, framer offsets, Costas scores, time-domain sweep positions). Setting a level shows that level and all more severe levels (e.g. **INFO** also shows **WARNING** and **FATAL**). Log output SHALL be written exclusively to the application's standard error stream in a consistent single-line format: `[OpenWSFZ] YYYY-MM-DD HH:MM:SS [LEVEL]  ComponentName — message`. Every component of the application — audio capture pipeline, FT8 decode pipeline (CycleFramer, Ft8Decoder, symbol extraction, Costas synchronisation, LDPC), WebSocket event bus, HTTP API, and configuration store — SHALL emit log entries at the appropriate level at key operational points sufficient to diagnose the application state without a debugger attached. The log level setting SHALL take effect on the next application start. | Must Have   |
...
| 1.4     | 2026-05-22 | QA (AI-assisted)                      | Added **FR-019** (configurable application logging) — five-level scheme (OFF / FATAL / WARNING / INFO / DEBUG), output to stderr, format `[OpenWSFZ] YYYY-MM-DD HH:MM:SS [LEVEL]  Component — message`, persisted as `AppConfig.LogLevel` (default `"INFO"`), exposed via Settings dropdown. Updated &sect;4.4 configuration field list. |
